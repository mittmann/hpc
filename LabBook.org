...
#+begin_src sh :results output :session :exports both :tangle scripts/Screening_run.sh
  #!/bin/bash
...
This is the code from 2016-07-19 with the following modifications:
    - rand() is now seeded with 0 instead of time(NULL) in order to
      achieve better reproducibility having the same numbers in every
      execution.
    - the matrix size is now a parameter received from the command
      line
#+begin_src C :results output :session :exports both :tangle mm.c
