...
#+begin_src sh :results output :session :exports both :tangle scripts/test_mmv1.sh
#!/bin/sh
#!/bin/bash
...
#+begin_src bash :results output :session :exports both :tangle scripts/combine_mmv1
#!/bin/bash
while IFS="," read f1 f2 f3 f4 f5 f6 f7 f8 
do
        if [ "$f1" != "\"name\"" ]; then
...
* 2016-08-22 Analysis of =MM_output.csv= file (compiler comparison)     :Lucas:
...
[1] "name"                "run.no.in.std.order" "run.no"             
[4] "run.no.std.rp"       "size"                "threads"            
[7] "compiler"            "time"
#+end_example
...
#+RESULTS:
#+begin_example
   size threads compiler  N       mean          se
1   512       1    clang 10  0.2847192 0.001748339
2   512       1      gcc 10  0.3152215 0.028917110
3   512       2    clang 10  0.1526065 0.012157844
4   512       2      gcc 10  0.1696786 0.022556112
5   512       4    clang 10  0.1079303 0.009120293
6   512       4      gcc 10  0.1093200 0.011652598
7   512       8    clang 10  0.1124349 0.009786947
8   512       8      gcc 10  0.1165455 0.008749708
9   724       1    clang 10  0.9166203 0.001170855
10  724       1      gcc 10  0.9996797 0.042677035
11  724       2    clang 10  0.5026853 0.033442052
12  724       2      gcc 10  0.5256714 0.041661816
13  724       4    clang 10  0.3435932 0.024998226
14  724       4      gcc 10  0.3415436 0.026252063
15  724       8    clang 10  0.3507483 0.018702872
16  724       8      gcc 10  0.3468855 0.030822741
17 1024       1    clang 10  3.8464633 0.002203172
18 1024       1      gcc 10  4.0134046 0.007968973
19 1024       2    clang 10  2.0854024 0.039283208
20 1024       2      gcc 10  2.1866821 0.057622485
21 1024       4    clang 10  1.3218843 0.054698495
22 1024       4      gcc 10  1.2644383 0.060800525
23 1024       8    clang 10  1.3078045 0.057953393
24 1024       8      gcc 10  1.3121323 0.044958083
25 1448       1    clang 10 11.4506607 0.002331510
26 1448       1      gcc 10 11.9472751 0.025798115
27 1448       2    clang 10  6.1661018 0.084228929
28 1448       2      gcc 10  6.4806272 0.151669038
29 1448       4    clang 10  3.8284503 0.204192751
30 1448       4      gcc 10  3.7774774 0.184568996
31 1448       8    clang 10  4.0827437 0.138143768
32 1448       8      gcc 10  4.0535519 0.176081777
#+end_example

Plot

#+begin_src R :results output graphics :file img/MM_output.png :exports both :width 600 :height 400 :session
library(ggplot2);
ggplot(k[k$size == 1448,], aes(x=threads, y=mean, color=compiler)) +
    geom_point() +
    geom_errorbar(aes(ymin=mean-se, ymax=mean+se), width=.2) +
    theme_bw();
#    facet_grid(size~compiler, scales="free_y");
...
#+begin_src R :results output :session :exports both :tangle MM_Scenario_O0.r
  require(DoE.base);
...
[1] "name"                "run.no.in.std.order" "run.no"             
[4] "run.no.std.rp"       "threads"             "compiler"           
[7] "time"
#+end_example
...
#+begin_src R :results output graphics :file img/MM_output_O0.png :exports both :width 600 :height 400 :session
library(ggplot2);
ggplot(aes(x=threads, y=mean, color=compiler)) +
    geom_point() +
    geom_errorbar(aes(ymin=mean-se, ymax=mean+se), width=.2) +
    theme_bw();
#+end_src
